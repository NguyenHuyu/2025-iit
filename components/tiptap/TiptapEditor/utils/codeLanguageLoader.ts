...
export const languagesLoader: Record<string, () => Promise<unknown>> = {
...
export async function loadLanguage(
    ...
    try {
        const { default: language } = await (languagesLoader[languageName]?.() as Promise<{
            default: any
        }>)
...
export function findLanguage(value: string | undefined) {
    ...
    const language = CODE_BLOCK_LANGUAGUES.find(
        (language) =>
            language.syntax === lowercase || language.alias?.split(', ').includes(lowercase)
    )
    return language
}
