  ...
  return (
    <NodeViewWrapper className="toggle-list my-3">
      <div className="flex items-start gap-2">
        <button
          className={`mt-1 flex h-5 w-5 items-center justify-center rounded-sm transition-transform ${isOpen ? "rotate-90" : ""}`}
          onClick={toggleOpen}
          type="button"
        >
        ...
        <div className="flex-1">
          ...
          />
          <div className={`toggle-content mt-1 pl-2 ${isOpen ? "block" : "hidden"}`} data-toggle-content />
        </div>
      </div>
    </NodeViewWrapper>
  )
