...
const BlocksMenu = ({ editor }: BlocksMenuProps) => {
  ...
  const blockOptions: BlockOption[] = [
    // Basic blocks
    {
      ...
      icon: <Paragraph className="h-5 w-5" />,
          ...
          <div key={section} className="mb-4">
            ...
            <div className="space-y-1">
              {blockOptions
                ...
                .map((item, index) => {
                  const itemIndex = blockOptions.findIndex((option) => option.title === item.title)
                  return (
                    <button
                      key={item.title}
                      className={cn(
                        "flex w-full items-center rounded-md px-2 py-1.5 text-sm hover:bg-gray-100 dark:hover:bg-gray-800",
                        selectedIndex === itemIndex && "bg-gray-100 dark:bg-gray-800",
                      )}
                      onClick={() => {
                        item.command(editor)
                        hideMenu()
                    ...
                    >
                      <div className="mr-2 flex h-10 w-10 items-center justify-center rounded-md border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
                        {item.icon}
                      </div>
